...
// =========================================================================
// 1. Esquema de Usuario en Mongoose actualizado y flexible
const usuarioSchema = new mongoose.Schema({
    correo: { type: String, required: true, unique: true, lowercase: true },
    contrasena: { type: String, required: true },
    rol: { type: String, required: true, enum: ['PROVEEDOR', 'COMPRADOR'] },
    nombre: { type: String, required: true },
    ruc: { type: String, default: "081-301205-1001N" },
    direccion: { type: String, default: "Managua, Nicaragua" },
    telefono: { type: String, default: "88888888" },
...
// 2. Ruta de Registro completamente tolerante a fallos
app.post('/api/registro', async (req, res) => {
    try {
        const { correo, contrasena, rol, nombre, ruc } = req.body;
        ...
        if (existe) {
            return res.status(400).json({ exito: false, error: "Este correo ya está registrado en el sistema." });
        }
        ...
        await Usuario.create({
            ...
            rol,
            nombre: nombre.trim(),
            ruc: ruc ? ruc.trim() : "081-301205-1001N",
            estado: "PENDIENTE_VERIFICACION",
            codigo_verificacion: tokenOTP
        });

        // Intentar envío de correo SMTP (Brevo) de forma segura sin romper el registro si falla
        try {
            const transporter = configurarTransporterB2B();
            if (transporter) {
                await transporter.sendMail({
                    from: '"SupplierNi B2B" <henrylechado41@gmail.com>',
                    to: correoLimpio,
                    subject: `🔐 Tu Código OTP de Activación: ${tokenOTP}`,
                    html: `<h3>Bienvenido a SupplierNi</h3><p>Tu código OTP de 6 dígitos es: <strong>${tokenOTP}</strong></p>`
                });
            }
        } catch (mailErr) {
            console.log("[AVISO SMTP] No se pudo enviar el correo físico, pero el token está activo:", mailErr.message);
        }
        ...
        console.error("ERROR CRÍTICO EN /api/registro:", err);
        res.status(500).json({ exito: false, error: "Error interno al procesar el registro: " + err.message });
    }
});
